...
export class CategoryService {
  ...
  async findAll({count, page}: PaginationDto) {
    const categories = await this.categoryModel.find({}, {}, {limit: count, skip: page * count - count, sort: {name: 1}})
  ...
  async update(id: string, dto: CreateCategoryDto) {
    ...
    const oldCategory = await this.categoryModel.findOne({name: dto.name})
    if(oldCategory) throw new BadRequestException("Такая категория уже существует")
